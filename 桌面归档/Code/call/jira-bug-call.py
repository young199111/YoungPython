...
def main():
    ...
    # 客端CRM
    all_list = [
        'issuetype = "缺陷(Bug)" AND status in ("[BUG]待修复", "[通用]挂起", "[Bug]待回归", "[Bug]正常关闭", "[Bug]待测试确认", "[Bug]待产品确认") AND text ~ 【呼叫1.1】 AND created >= 2022-11-01 AND reporter in (lulan, shijiaqi, lijingjing, yanmin, zhanglin1, geyaru, wangweina, wangweina2, wangyang7) ORDER BY status ASC, cf[11109] DESC, assignee ASC'
    ]
    fixed_list = [
        'issuetype = "缺陷(Bug)" AND status in ("[通用]挂起", "[Bug]待回归", "[Bug]正常关闭") AND text ~ 【呼叫1.1】 AND created >= 2022-11-01 AND reporter in (lulan, shijiaqi, lijingjing, yanmin, zhanglin1, geyaru, wangweina, wangweina2, wangyang7) ORDER BY status ASC, cf[11109] DESC, assignee ASC'
    ]
    to_fix_list = [
        'issuetype = "缺陷(Bug)" AND status in ("[BUG]待修复") AND text ~ 【呼叫1.1】 AND created >= 2022-11-01 AND reporter in (lulan, shijiaqi, lijingjing, yanmin, zhanglin1, geyaru, wangweina, wangweina2, wangyang7) ORDER BY status ASC, cf[11109] DESC, assignee ASC'
    ]
    to_check_list = [
        'issuetype = "缺陷(Bug)" AND status in ("[Bug]待测试确认", "[Bug]待产品确认") AND text ~ 【呼叫1.1】 AND created >= 2022-11-01 AND reporter in (lulan, shijiaqi, lijingjing, yanmin, zhanglin1, geyaru, wangweina, wangweina2, wangyang7) ORDER BY status ASC, cf[11109] DESC, assignee ASC'
    ]
