...
def main():
    ...
    # date, "**时间**\n2021-07-25 15:35:00"
    # reference
    req_name = "SaaS进退场（0117）"
    date_time = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
    groups = [
        ('409bf94b-92d8-43de-beac-0671ecf9edef', '**需求**\n' + req_name, '**时间**\n' + date_time,
         '[数据来源Jira](https://jira.znlhzl.org/issues/?filter=13399)')
    ]
    ...
    # 客端CRM
    all_list = [
        'issuetype = "缺陷(Bug)" AND status in ("[BUG]待修复", "[通用]挂起", "[Bug]待回归", "[Bug]正常关闭", "[Bug]待测试确认", "[Bug]待产品确认") AND text ~ 【进退场改造】 AND created >= 2023-01-07 AND reporter in (shijiaqi, lijingjing, yanmin, geyaru, wangweina, wangweina2, wangyang7) ORDER BY status ASC, cf[11109] DESC, assignee ASC'
    ]
    fixed_list = [
        'issuetype = "缺陷(Bug)" AND status in ("[通用]挂起", "[Bug]待回归", "[Bug]正常关闭") AND text ~ 【进退场改造】 AND created >= 2023-01-07 AND reporter in (shijiaqi, lijingjing, yanmin, geyaru, wangweina, wangweina2, wangyang7) ORDER BY status ASC, cf[11109] DESC, assignee ASC'
    ]
    to_fix_list = [
        'issuetype = "缺陷(Bug)" AND status in ("[BUG]待修复") AND text ~ 【进退场改造】 AND created >= 2023-01-07 AND reporter in (shijiaqi, lijingjing, yanmin, geyaru, wangweina, wangweina2, wangyang7) ORDER BY status ASC, cf[11109] DESC, assignee ASC'
    ]
    to_check_list = [
        'issuetype = "缺陷(Bug)" AND status in ("[Bug]待测试确认", "[Bug]待产品确认") AND text ~ 【进退场改造】 AND created >= 2023-01-07 AND reporter in (shijiaqi, lijingjing, yanmin, geyaru, wangweina, wangweina2, wangyang7) ORDER BY status ASC, cf[11109] DESC, assignee ASC'
    ]
