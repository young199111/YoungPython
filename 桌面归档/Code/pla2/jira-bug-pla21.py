...
def main():
    ...
    # date, "**时间**\n2021-07-25 15:35:00"
    # reference
    req_name = "联营结算二期-审批流（0202）"
    date_time = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
    groups = [
        ('f1a5df6b-53e2-49f4-9c51-d28e0426b947', '**需求**\n' + req_name, '**时间**\n' + date_time,
         '[数据来源Jira](https://jira.znlhzl.org/issues/?filter=13402)')
    ]
    ...
    # 客端CRM
    all_list = [
        'issuetype = "缺陷(Bug)" AND status in ("[BUG]待修复", "[通用]挂起", "[Bug]待回归", "[Bug]正常关闭", "[Bug]待测试确认", "[Bug]待产品确认") AND text ~ 【审批流】 AND created >= 2023-01-07 ORDER BY status ASC, cf[11109] DESC, assignee ASC'
    ]
    fixed_list = [
        'issuetype = "缺陷(Bug)" AND status in ("[通用]挂起", "[Bug]待回归", "[Bug]正常关闭") AND text ~ 【审批流】 AND created >= 2023-01-07 ORDER BY status ASC, cf[11109] DESC, assignee ASC'
    ]
    to_fix_list = [
        'issuetype = "缺陷(Bug)" AND status in ("[BUG]待修复") AND text ~ 【审批流】 AND created >= 2023-01-07 ORDER BY status ASC, cf[11109] DESC, assignee ASC'
    ]
    to_check_list = [
        'issuetype = "缺陷(Bug)" AND status in ("[Bug]待测试确认", "[Bug]待产品确认") AND text ~ 【审批流】 AND created >= 2023-01-07 ORDER BY status ASC, cf[11109] DESC, assignee ASC'
    ]
