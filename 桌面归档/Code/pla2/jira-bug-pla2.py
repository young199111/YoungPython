...
def main():
    ...
    # date, "**时间**\n2021-07-25 15:35:00"
    # reference
    req_name = "联营结算二期（0129）"
    date_time = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
    groups = [
        ('7058341a-86c3-454f-98ab-489ff5cdb8a4', '**需求**\n' + req_name, '**时间**\n' + date_time,
         '[数据来源Jira](https://jira.znlhzl.org/issues/?filter=13397)')
    ]
    ...
    # 客端CRM
    all_list = [
        'issuetype = "缺陷(Bug)" AND status in ("[BUG]待修复", "[通用]挂起", "[Bug]待回归", "[Bug]正常关闭", "[Bug]待测试确认", "[Bug]待产品确认") AND text ~ 【联营结算二期】 AND created >= 2023-01-07 ORDER BY status ASC, cf[11109] DESC, assignee ASC'
    ]
    fixed_list = [
        'issuetype = "缺陷(Bug)" AND status in ("[通用]挂起", "[Bug]待回归", "[Bug]正常关闭") AND text ~ 【联营结算二期】 AND created >= 2023-01-07 ORDER BY status ASC, cf[11109] DESC, assignee ASC'
    ]
    to_fix_list = [
        'issuetype = "缺陷(Bug)" AND status in ("[BUG]待修复") AND text ~ 【联营结算二期】 AND created >= 2023-01-07 ORDER BY status ASC, cf[11109] DESC, assignee ASC'
    ]
    to_check_list = [
        'issuetype = "缺陷(Bug)" AND status in ("[Bug]待测试确认", "[Bug]待产品确认") AND text ~ 【联营结算二期】 AND created >= 2023-01-07 ORDER BY status ASC, cf[11109] DESC, assignee ASC'
    ]
