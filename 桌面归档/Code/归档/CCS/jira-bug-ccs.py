...
def main():
    ...
    # 客端CRM
    all_list = [
        'issuetype = "缺陷(Bug)" AND status in ("[BUG]待修复", "[Bug]待回归", "[Bug]正常关闭", "[Bug]待测试确认") AND text ~ "【线索" AND created >= 2022-07-20 AND reporter in (wangyang7, yanmin, lulan, lijingjing, geyaru) ORDER BY status ASC'
    ]
    fixed_list = [
        'issuetype = "缺陷(Bug)" AND status in ("[Bug]待回归", "[Bug]正常关闭") AND text ~ "【线索" AND created >= 2022-07-20 AND reporter in (wangyang7, yanmin, lulan, lijingjing, geyaru) ORDER BY status ASC'
    ]
    to_fix_list = [
        'issuetype = "缺陷(Bug)" AND status = "[BUG]待修复" AND text ~ "【线索" AND created >= 2022-07-20 AND reporter in (wangyang7, yanmin, lulan, lijingjing, geyaru) ORDER BY status ASC'
    ]
    to_check_list = [
        'issuetype = "缺陷(Bug)" AND status = "[Bug]待测试确认" AND text ~ "【线索" AND created >= 2022-07-20 AND reporter in (wangyang7, yanmin, lulan, lijingjing, geyaru) ORDER BY status ASC'
    ]
