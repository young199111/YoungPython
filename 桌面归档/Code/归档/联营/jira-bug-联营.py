...
def main():
    ...
    groups = [
        # 联营
        ('0aef4be9cc86b6b703638d5f4520d62a56a8bc65b8685c079153e0ebaaacc82e',
         'SEC4bd10d972ac0cb94c305473ce6dac02aae594ec867ce98372a2e0aacfed1f922')
    ]
    ...
    # 客端CRM
    all_list = [
        'issuetype = "缺陷(Bug)" AND status in ("[BUG]待修复", "[通用]挂起", "[Bug]待回归", "[Bug]正常关闭", "[Bug]待测试确认") AND text ~ "联营" AND created >= 2022-08-08 AND created <= 2022-08-31 ORDER BY created DESC'
    ]
    fixed_list = [
        'issuetype = "缺陷(Bug)" AND status in ("[通用]挂起", "[Bug]待回归", "[Bug]正常关闭") AND text ~ "联营" AND created >= 2022-08-08 AND created <= 2022-08-31 ORDER BY created DESC'
    ]
    to_fix_list = [
        'issuetype = "缺陷(Bug)" AND status = "[BUG]待修复" AND text ~ "联营" AND created >= 2022-08-08 AND created <= 2022-08-31 ORDER BY created DESC'
    ]
    to_check_list = [
        'issuetype = "缺陷(Bug)" AND status = "[Bug]待测试确认" AND text ~ "联营" AND created >= 2022-08-08 AND created <= 2022-08-31 ORDER BY created DESC'
    ]
