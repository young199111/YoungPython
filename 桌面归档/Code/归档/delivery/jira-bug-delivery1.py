...
def main():
    ...
    groups = [
        # CCS
        ('005f57c916c8b6cede207477bc9c2169d9cf2277fc46b71c26fb2ce0e9395d98',
         'SEC4457351024ee41a7bac326aab0d89dc522c9fc1526f6bc6523afb20038ca4c9b')
    ]
    ...
    # 客端CRM
    all_list = [
        'issuetype = "缺陷(Bug)" AND status in ("[BUG]待修复", "[通用]挂起", "[Bug]待回归", "[Bug]正常关闭", "[Bug]待测试确认", "[Bug]待产品确认") AND text ~ 【居间履约迁移】 AND created >= 2022-09-29 AND reporter in (shijiaqi, lijingjing, yanmin, geyaru, wangweina, wangweina2, wangyang7) ORDER BY status ASC, cf[11109] ASC, assignee ASC'
    ]
    fixed_list = [
        'issuetype = "缺陷(Bug)" AND status in ("[通用]挂起", "[Bug]待回归", "[Bug]正常关闭") AND text ~ 【居间履约迁移】 AND created >= 2022-09-29 AND reporter in (shijiaqi, lijingjing, yanmin, geyaru, wangweina, wangweina2, wangyang7) ORDER BY status ASC, cf[11109] ASC, assignee ASC'
    ]
    to_fix_list = [
        'issuetype = "缺陷(Bug)" AND status = "[BUG]待修复" AND text ~ 【居间履约迁移】 AND created >= 2022-09-29 AND reporter in (shijiaqi, lijingjing, yanmin, geyaru, wangweina, wangweina2, wangyang7) ORDER BY status ASC, cf[11109] ASC, assignee ASC'
    ]
    to_check_list = [
        'issuetype = "缺陷(Bug)" AND status in ("[Bug]待测试确认", "[Bug]待产品确认") AND text ~ 【居间履约迁移】 AND created >= 2022-09-29 AND reporter in (shijiaqi, lijingjing, yanmin, geyaru, wangweina, wangweina2, wangyang7) ORDER BY status ASC, cf[11109] ASC, assignee ASC'
    ]
