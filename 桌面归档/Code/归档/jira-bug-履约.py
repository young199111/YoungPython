...
def main():
    ...
    groups = [
        # 履约
        ('0fade0956f9ea5926892ba4df9710610f38087955acced165d761eebee31f8c4',
         'SEC656da76d23bd3c814f0ad741b75e39d3d845b9ca185ae200fd522c19d3fae17f')
    ]
    ...
    # 履约
    fixed_list = [
        'issuetype = "缺陷(Bug)" AND status in ("[Bug]待回归", "[Bug]正常关闭") AND text ~ "【履约" AND created >= 2022-07-18 AND created <= 2022-07-29 AND reporter in (yanmin, wangweina2, wangweina) ORDER BY status ASC'
    ]
    to_fix_list = [
        'issuetype = "缺陷(Bug)" AND status in ("[BUG]待修复", "[Bug]待测试确认") AND text ~ "【履约" AND created >= 2022-07-18 AND created <= 2022-07-29 AND reporter in (yanmin, wangweina2, wangweina) ORDER BY status ASC'
    ]
